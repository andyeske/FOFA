# ------------------------------------------------ #
# Fuel-Optimal Aircraft Allocation (FOAA) - Yearly Basis (YB)
# ------------------------------------------------ #
# DESCRIPTION - FOAA: 
...
#
# VERSION - YEARLY BASIS:
# This code implements the framework on a yearly-basis 
# (i.e., all 366 days of 2024 are collapsed into one),
# so to enable conducting a sensitivity analysis based
# on the system-wide load factor.
# ------------------------------------------------ #
...
average_yearly_LF = np.round(RPKs/ASKs,3)

# Load Factor Sensitivity Analysis
#LF_vec = [0.8,0.82,average_yearly_LF,0.84,0.86,0.88,0.9,0.92,0.94,0.96,0.98,1]
...
for target_LF in LF_vec:
    ...
    constraint_1 = m.addConstr(x@aircraft_seats >= yearly_passengers)
    
    # 2) At least a minimum system-wide load factor must be attained
    new_ASKs = np.transpose(route_distance)@x@aircraft_seats
    constraint_2 = m.addConstr(RPKs/1000 <= target_LF*new_ASKs)
    
    # 3) The total flying time can't exceed the actual flying time
    constraint_3 = m.addConstr(np.transpose(flight_time)@x <= np.transpose(yearly_aircraft_flying_times))
    ...
    # Storing values specific to the base case
    if LF_in == 2:
        # Calculating the average stage length
        stage_lengths = np.zeros([20,1])
        stage_lengths[0:19,0] = aircraft_stage_length
        stage_lengths[0:19,1] = np.transpose(np.transpose(route_distance)@x_solution/np.sum(x_solution,0))*1000
