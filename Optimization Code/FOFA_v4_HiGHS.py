# ------------------------------------------------ #
# Fuel-Optimal Fleet Assignment (FOFA) Model - V4
# ------------------------------------------------ #
# OPTIMIZATION SOLVER:
# HiGHS with OR-Tools
#
...
file_path = '/Users/andyeske/Desktop/Fall 2025/Optimization Methods/Project/Processed Datasets/Aircraft Statistics.xlsx'; 
aircraft_performance = pd.read_excel(file_path,sheet_name='Performance'); 
aircraft_range = aircraft_performance['Range (km)'].to_numpy(); aircraft_range = aircraft_range[0:19]; aircraft_range = np.vstack(aircraft_range).astype(np.int64); aircraft_range = np.round(aircraft_range[:,0],3)
...
solver = pywraplp.Solver.CreateSolver('HiGHS')  # or 'CBC' for open-source

# Decision variables: number of frequencies on the O-D market operated by aircraft type a
x = {}
for r in range(l_routes):
    for a in range(l_aircraft):
        x[(r,a)] = solver.IntVar(0, solver.infinity(), f"x_{r}_{a}")

# Objective function: minimize the system-wide fuel consumption
objective = solver.Objective()
for r in range(l_routes):
    for a in range(l_aircraft):
        objective.SetCoefficient(x[(r,a)], route_distance[r] * aircraft_fuel[a])
...
for r in fixed_routes:
    for a in range(l_aircraft):
        solver.Add(x[(r,a)] >= baseline_fleet_assignment[r,a])
...
for r in range(l_routes):
    solver.Add(sum(aircraft_seats[a] * x[(r,a)] for a in range(l_aircraft)) >= total_passengers[r])
num_constraints = num_constraints + non_zero_routes # Updating constraints count

# 2) At least the same number of flights on each route must be preserved
for r in range(l_routes):
    solver.Add(sum(x[(r,a)] for a in range(l_aircraft)) >= total_departures[r])
num_constraints = num_constraints + non_zero_routes # Updating constraints count

# 3) At least a minimum system-wide load factor must be attained
new_ASKs = sum(route_distance[r] * aircraft_seats[a] * x[(r,a)]
               for r in range(l_routes)
               for a in range(l_aircraft))
solver.Add(RPKs <= max_LF * new_ASKs / 100)
num_constraints = num_constraints + 1 # Updating constraints count

# 4) The total flying time can't exceed the actual flying time
for a in range(l_aircraft):
    solver.Add(sum(flight_time[r] * x[(r,a)] for r in range(l_routes))<= max_avail * total_aircraft_flying_times[a] / 100)
num_constraints = num_constraints + non_zero_aircraft # Updating constraints count

# 5) Aircraft must possess sufficient range to cover the route
for r in range(l_routes):
    for a in range(l_aircraft):
        if route_distance[r] > aircraft_range[a]:
            solver.Add(x[(r,a)] == 0)
for a in range(0,np.size(non_zero_aircraft)): # Updating constraints count
...
for node in range(169):
    o_in = np.argwhere(Airport_In == node).flatten()
    d_in = np.argwhere(Airport_Out == node).flatten()
    if len(o_in) > 0:
        solver.Add(sum(x[(r,a)] for r in o_in for a in range(l_aircraft)) == sum(x[(r,a)] for r in d_in for a in range(l_aircraft)))
        num_constraints = num_constraints + 1 # Updating constraints count
...
for node in range(169):
    ...
    d_in = np.argwhere(Airport_Out == node).flatten()
    for a in range(l_aircraft):
        if aircraft_runways[a] > runway:
            for r in o_in:
                solver.Add(x[(r,a)] == 0)
            for r in d_in:
                solver.Add(x[(r,a)] == 0)
    
    runway_in = np.nonzero(1*(aircraft_runways[aircraft_in] > runway))[0] # Updating constraints count
    num_constraints = num_constraints + len(runway_in)

# Solving the optimization problem
solver_parameters = pywraplp.MPSolverParameters()
solver_parameters.SetDoubleParam(pywraplp.MPSolverParameters.RELATIVE_MIP_GAP, 0.01) # Optimality Gap = 0.1%
status = solver.Solve(solver_parameters)
solver.SuppressOutput()

# Extracting the solution
x_solution = np.zeros([l_routes, l_aircraft])
if status in [pywraplp.Solver.OPTIMAL, pywraplp.Solver.FEASIBLE]:
    for r in range(l_routes):
        for a in range(l_aircraft):
            x_solution[r,a] = x[(r,a)].solution_value()
run_time = time.perf_counter()

# Computing the optimality gap
best_solution = solver.Objective().Value()
best_bound = solver.Objective().BestBound()
gap = abs(best_bound - best_solution) / abs(best_solution)
...
with warnings.catch_warnings():
    ...
    # Calculating the constraints slack
    ...
    slack_uti = np.transpose(max_avail*total_aircraft_flying_times/100) - np.transpose(flight_time)@x_solution
    slack_uti_per = np.round(100*slack_uti/np.transpose(max_avail*total_aircraft_flying_times/100),2)
    
# Calculting the new fuel intensity (FI)
original_FI = np.round(original_fuel_consumption/RPKs,3)
new_FI = np.round(new_fuel_consumption/RPKs,3)
...
print("--> Maximum Aircraft Utilization: " + str(max_avail) + "%")
...
for k in range(0,19):
    if np.isnan(slack_uti_per[k]) == 0:
        if slack_uti_per[k] == 100:
            print("----> " + aircraft_names[k] + " Optimized ASL (km): 0 km " + "| 2024 ASL (km): " + str(int(np.round(avg_stage_length_vec_baseline[k,0]*1000))) + " km")
        else:
            print("----> " + aircraft_names[k] + " Optimized ASL (km): " + str(int(np.round(avg_stage_length_vec[k,0]*1000))) + " km " + "| 2024 ASL (km): " + str(int(np.round(avg_stage_length_vec_baseline[k,0]*1000))) + " km")
print("----> " + aircraft_names[19] + " Optimized ASL (km): " + str(int(np.round(avg_stage_length_vec[19,0]*1000))) + " km " + "| 2024 ASL (km): " + str(int(np.round(avg_stage_length_vec_baseline[19,0]*1000))) + " km")
print("")
print("-------------------- Optimization --------------------")        
print("--> Overall Statistics:")
print("----> Number of Variables: " + str(num_vars))
print("----> Number of Constraints: " + str(num_constraints))
print("----> Runtime (s): " + str(np.round(run_time - start_time,2)) + 's')
print("----> Optimality Gap (%): " + str(np.round(100*gap,2)) + '%')
print("")
